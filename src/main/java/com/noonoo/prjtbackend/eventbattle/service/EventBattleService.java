...
public interface EventBattleService {
    ...
    /** SSE 브로드캐스트용: 모든 클라이언트에 동일 JSON을 보내므로 myBet / myBetHistory(로그인 사용자 전용)는 포함하지 않습니다. */
    ...
    /** 이벤트 취소: - 모든 참가자 베팅 포인트 환불 - 이벤트를 CANCELLED 상태로 종료 */
    void cancel(long eventBattleSeq);

    /**
     * 투표 전용 이벤트: 투표만 마감하고 종료(SETTLED, 승리 주제 없음). 이후 {@link #vote(long, EventBattleVoteRequest)} 는
     * 불가합니다.
     */
