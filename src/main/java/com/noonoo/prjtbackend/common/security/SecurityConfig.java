 ...
 *
 * <ul>
 *   <li>{@code app.security.permit-all=true} (기본): 필터 체인 전체 허용 + {@link SecurityExpressions}로 메서드
 *       권한도 통과 (로컬 개발)
 *   <li>{@code false}: 로그인({@code /api/auth/login}) 후 세션 + DB 권한({@code MEMBER_READ} 등) 검사
 * </ul>
 */
@Configuration
@EnableWebSecurity
            ...
            http.authorizeHttpRequests(
                    auth ->
                            auth.requestMatchers(HttpMethod.OPTIONS, "/**")
                                    ...
                                    .permitAll()
                                    .requestMatchers(HttpMethod.GET, "/api/menus/sidebar")
                                    .permitAll()
                                    ...
                                    .permitAll()
                                    // 로컬·스테이징: OpenAPI JSON·Swagger UI·Scalar(/scalar/**). 운영
                                    // prod에서는 springdoc 비활성.
                                    .requestMatchers(
                                            "/v3/api-docs",
                                            "/v3/api-docs/**",
                                            "/swagger-ui/**",
