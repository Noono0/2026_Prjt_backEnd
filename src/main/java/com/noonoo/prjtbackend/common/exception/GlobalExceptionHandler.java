 ...
 * 전역 예외 처리기
 *
 * <p>컨트롤러/서비스에서 예외가 발생했을 때 공통 응답 형태로 변환해서 내려준다.
 */
@Slf4j
...
public class GlobalExceptionHandler {
    ...
    /**
     * {@code @PreAuthorize} 메서드 인가 실패 (@code SecurityExpressions} 등). 상세는 SecurityExpressions 의
     * WARN 로그(필요 authority)를 우선 확인.
     */
    ...
    public ResponseEntity<ApiResponse<Void>> handleAuthorizationDenied(
        ...
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ApiResponse.fail("FORBIDDEN", "이 작업을 수행할 권한이 없습니다. 메뉴(ROLE) 권한을 확인하세요."));
    }
    ...
    /**
     * DB 제약조건 오류 예: unique key 중복, not null 위반, fk 위반 등
     *
     * <p>DB 원문은 로그에 남기고, 프론트에는 가공된 메시지만 전달한다.
     */
    ...
    public ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolationException(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ApiResponse.fail("DATA_INTEGRITY_ERROR", "입력값이 올바르지 않거나 중복된 데이터가 있습니다."));
    }
    ...
    public ResponseEntity<ApiResponse<Void>> handleException(Exception e) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(
                        ApiResponse.fail(
                                "INTERNAL_SERVER_ERROR", "처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요."));
    }
}
