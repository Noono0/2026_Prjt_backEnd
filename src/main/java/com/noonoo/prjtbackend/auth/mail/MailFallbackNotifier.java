...
public class MailFallbackNotifier {
    ...
    public void notifyPasswordResetCode(String to, String memberId, String code) {
        if (logCodeInDev) {
            log.warn(
                    "[DEV] SMTP 미설정 — 비밀번호 재설정 인증코드 | to={} memberId={} code={}",
                    maskEmail(to),
                    memberId,
                    code);
            return;
        ...
        throw new IllegalArgumentException(
                "이메일 발송(SMTP)이 설정되지 않았습니다. application.yml 의 spring.mail.* 를 구성하거나 app.mail.log-code-in-dev=true 로 로컬 테스트 해 주세요.");
    }
