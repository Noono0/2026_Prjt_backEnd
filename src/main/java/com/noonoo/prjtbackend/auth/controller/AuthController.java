...
public class AuthController {
    ...
    /**
     * 로그인: Spring Security {@link SecurityContextHolder} 에 인증을 올려 세션 기반으로 이후 API의
     * {@code @PreAuthorize} / DB 권한이 동작합니다.
     */
    @PostMapping("/login")
        ...
        Authentication authentication =
                authenticationManager.authenticate(
                        new UsernamePasswordAuthenticationToken(
                                request.getMemberId(), request.getMemberPwd()));
    ...
    /**
     * NextAuth OAuth 로그인 직후 서버(BFF)에서만 호출 — {@code X-OAuth-Sync-Secret} 와 {@code OAUTH_SYNC_SECRET}
     * 일치 필요. 신규 시 member 행 + MEMBER_ROLE(USER) + 가입 포인트, 기존 시 프로필/동기화 시각 갱신.
     */
    ...
    /**
     * NextAuth 세션과 동일한 회원으로 Spring HttpSession + SecurityContext 를 맞춤. 브라우저는 Next 의
     * 프록시(`/api/auth/spring-sync`)만 호출하고, Secret 은 서버 간 공유.
     */
    ...
    public ApiResponse<TokenResponse> oauthEstablishSession(
        ...
        if (m == null || m.getMemberId() == null || !m.getMemberId().equals(body.getMemberId())) {
    ...
    public ApiResponse<Void> passwordResetRequest(
            @RequestBody PasswordResetRequestDto body, HttpServletRequest request) {
        passwordResetService.requestCode(body, request);
        return ApiResponse.ok(PW_RESET_SENT_MSG, null);
    }

    @PostMapping("/password-reset/verify")
    public ApiResponse<PasswordResetVerifyResponseDto> passwordResetVerify(
            @RequestBody PasswordResetVerifyDto body) {
    ...
    public ApiResponse<Void> passwordResetComplete(
            @RequestBody PasswordResetCompleteDto body, HttpServletRequest request) {
